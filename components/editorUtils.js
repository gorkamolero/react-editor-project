...
[CASE 1]: always split on two consecutive newlines, when cursor at end of the tweet
---
line 1
|
| < split on ⮐
...
[CASE 2]: split on 4 consecutive newlines when between paragraphs inside a tweet
---
line 1
|
| < spilt on ⮐
    ...
    for (let tIndex = 0; tIndex < content.length; tIndex++) {
      ...
      if (shouldSmartSplitTweet(tweet)) {
        ...
        if (tweet.content.length > 1) {
          ...
          pForOldTweet = [
            { type: 'paragraph', content: [{ type: 'text', text: oldText }] }
          ]
          pForNewTweet = [
